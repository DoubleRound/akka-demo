...
/**
 * 发送者，其实这个相当于Pi中的Master，来分派任务给worker，接收处理worker返回的结果
 **/
object Sender {
  ...
  def props(path: String, totalMessages: Int, burstSize: Int, payloadSize: Int): Props = Props(new Sender(path, totalMessages, burstSize, payloadSize))
